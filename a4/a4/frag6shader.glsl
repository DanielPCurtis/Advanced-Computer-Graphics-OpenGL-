...
{	// normalize the light, normal, and view vectors:
	...
	vec4 texColor = texture(s,originalPosition);
	...
	fragColor = (0.5 *
				...
				(0.5 *
				(globalSpot * matAmb  +  light2.ambient * matAmb
				+ light2.diffuse * matDiff * max(cosTheta,0.0)
				+ light2.specular * matSpec * pow(max(cosPhi,0.0), material.shininess))
				+
				(0.5 * texColor));
}
